...
pruned_model_fname = os.path.join('result', f'activation_pruned_model_sparsity{sparsity_level}.keras')
...
filters_to_prune_per_batch = [[] for _ in activation_layer_names]
...
for i in range(0, num_samples, batch_size):
    ...
    for layer_idx, activation in enumerate(batch_activations):
        ...
        # determine the threshold for the lowest percentage of the activations in the batch
        pruning_threshold = np.percentile(mean_activations, sparsity_level)
        filters_to_prune = mean_activations < pruning_threshold
        filters_to_prune_per_batch[layer_idx].append(filters_to_prune)

# determine the filters to prune by overlapping across all batches
final_filters_to_prune = []

for layer_idx, layer_filters_to_prune in enumerate(filters_to_prune_per_batch):
    # convert to numpy array for easier manipulation
    layer_filters_to_prune = np.array(layer_filters_to_prune)

    # count how many times each filter is marked for pruning across all batches
    filter_prune_counts = np.sum(layer_filters_to_prune, axis=0)

    # calculate the threshold for the highest percentage of overlapping filters
    overlap_pruning_threshold = np.percentile(filter_prune_counts, 100 - sparsity_level)

    # determine the final filters to prune based on the threshold
    final_filters_to_prune_layer = filter_prune_counts >= overlap_pruning_threshold
    final_filters_to_prune.append(final_filters_to_prune_layer)


def prune_filters(convolutional_layer, final_filters_to_prune):
    ...
    weights[:, :, :, final_filters_to_prune] = 0
...
for i, conv_layer_name in enumerate(convolutional_layer_names):
    ...
    prune_filters(conv_layer, final_filters_to_prune[i])
